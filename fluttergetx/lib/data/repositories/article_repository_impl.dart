...
class ArticleRepositoryImpl implements ArticleRepository {
  ...
  Future<List<ArticleEntity>> getAll({int page = 1, int limit = 10}) async {
    ...
    final response = await _dio.get(uri, options: await _authHeaders());
  ...
  Future<ArticleEntity> create(ArticleEntity article, {String? imagePath}) async {
    ...
    final response = await _dio.post(uri, data: formData, options: (await _authHeaders())..contentType = 'multipart/form-data');
  ...
  Future<ArticleEntity> update(String id, ArticleEntity article, {String? imagePath}) async {
    ...
    final response = await _dio.put(uri, data: body, options: await _authHeaders());
    if (response.statusCode == 200) {
  ...
  Future<void> delete(String id) async {
    final uri = '$_baseUrl/articles/$id';
    final response = await _dio.delete(uri, options: await _authHeaders());
    if (response.statusCode != 200) {
